...
struct Mesh {
    ...
    void buildVertexArray(){
        ...
        //passer données vertices
        

        for(int i=0;i<vertices.size();i++){
            verticesArray.push_back(vertices[i][0]);
            verticesArray.push_back(vertices[i][1]);
            verticesArray.push_back(vertices[i][2]);
            normalsArray.push_back(normals[i][0]);
            normalsArray.push_back(normals[i][1]);
            normalsArray.push_back(normals[i][2]);
        }
        glVertexPointer(3,GL_FLOAT, 3*sizeof(float),(GLvoid*)(&this->verticesArray[0]));
        glNormalPointer(GL_FLOAT, 3*sizeof(float),(GLvoid*)(&this->normalsArray[0]));
        glColorPointer(3,GL_FLOAT,3*sizeof(float),(GLvoid*)(&this->colorsArray[0]));
    }


    void buildTriangleArray(){
        for(int i=0;i<triangles.size();i++){
            trianglesArray.push_back(triangles[i][0]);
            trianglesArray.push_back(triangles[i][1]);
            trianglesArray.push_back(triangles[i][2]);
            
        }
    }

    void buildColorsArray(){
        for(int i=0;i<normals.size();i++){
            colorsArray.push_back(normals[i][0]);
            colorsArray.push_back(normals[i][1]);
            colorsArray.push_back(normals[i][2]);
        }
        glColorPointer(3,GL_FLOAT,3*sizeof(float),(GLvoid*)(&colorsArray[0]));
    }    
};
...
void drawTriangleMesh( Mesh const & i_mesh ) {
    glDrawElements(GL_TRIANGLES, mesh.trianglesArray.size(),GL_UNSIGNED_INT,(GLvoid*)(&mesh.trianglesArray[0]));
}
...
void draw () {
    ...
    static std::string text = std::to_string(0);
    frames++;
